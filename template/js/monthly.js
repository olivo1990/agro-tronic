...
(function($) {
	$.fn.extend({
		...
		$('#' + uniqueId).prepend('<div class="monthly-header"><div class="monthly-header-hidden">x</div><div class="monthly-header-title"></div></div>').append('<div class="monthly-event-list"></div>');
		...
		function setMonthly(m, y){
			...
			if (options.mode == 'event') {
				for(var i = 0; i < dayQty; i++) {
					...
					var mes = uniqueId.split("_");

					if(mes[1] < mes_act){
						$('#' + uniqueId + ' .monthly-day-wrap').append('<div href="#" class="m-d monthly-day monthly-day-event monthly-day-past"><div class="monthly-day-number">'+day+'</div><div class="monthly-indicator-wrap"></div></div>');
					}else{
						if(mes[1] == mes_act && day < dia_act){
							$('#' + uniqueId + ' .monthly-day-wrap').append('<div href="#" class="m-d monthly-day monthly-day-event monthly-day-past"><div class="monthly-day-number">'+day+'</div><div class="monthly-indicator-wrap"></div></div>');
						}else{
							$('#' + uniqueId + ' .monthly-day-wrap').append('<a href="#" class="m-d monthly-day monthly-day-event" data-number="'+day+'"><div class="monthly-day-number">'+day+'</div><div class="monthly-indicator-wrap"></div></a>');
							//$('#' + uniqueId + ' .monthly-event-list').html('<div class="monthly-list-item" id="'+uniqueId+'day'+day+'" data-number="'+day+'"><div class="monthly-event-list-date">'+dayNames[dayNamenum]+'<br>'+day+'</div></div>');
						}	
					}
			...
			if(options.weekStart == 'D' && firstDay != 7) {
				for(var i = 0; i < firstDay; i++) {
					$('#' + uniqueId + ' .monthly-day-wrap').prepend('<div class="m-d monthly-day-blank"><div class="monthly-day-number"></div></div>');
				}
			} else if (options.weekStart == 'L' && firstDay == 0) {
				for(var i = 0; i < 6; i++) {
					$('#' + uniqueId + ' .monthly-day-wrap').prepend('<div class="m-d monthly-day-blank" ><div class="monthly-day-number"></div></div>');
				}
			} else if (options.weekStart == 'L' && firstDay != 1) {
			...
			for(var i = 0; i < divs.length; i+=7) {
				cont_sem++;
			    divs.slice(i, i+7).wrapAll("<div class='monthly-week' id='week_"+cont_sem+"'></div>");
			}
		}
		...
		$(document.body).on('click', '#'+uniqueId+' .monthly-cal', function (e) {
			$(this).remove();
				...
				$('a.monthly-day').each(function (index) 
		        {  
		...
		$(document.body).on('click', '#'+uniqueId+' a.monthly-day', function (e) {
			...
			if(options.mode == 'event' && options.eventList == true) {
				var elemento = this;
				var whichDay = $(elemento).data('number');
				var fecha = currentYear+"-"+currentMonth+"-"+whichDay;
				GetFecha(fecha,elemento,whichDay,uniqueId);
		...
		$(document.body).on('click', '#'+uniqueId+' div.monthly-header-hidden', function (e) {
	        DeleteFecha(uniqueId);
			e.preventDefault();
		});
